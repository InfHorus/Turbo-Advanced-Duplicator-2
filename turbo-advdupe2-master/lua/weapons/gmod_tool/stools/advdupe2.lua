...
if(SERVER) then
	...
	function TOOL:RightClick(trace)
		...
		elseif trace.HitNonWorld then    --Area Copy is off
			...
			if dupe.Entities then
				--clear the dupe
				express.Send("AdvDupe2_RemoveGhosts", {removed = true}, ply)
				dupe.Entities = nil
				dupe.Constraints = nil
				express.Send("AdvDupe2_ResetDupeInfo", {reset = true}, ply)
		...
		express.Send("AdvDupe2_SetDupeInfo", {
			filename = " ",
			creator = ply:Nick() or " ",
			date = os.date("%d %B %Y") or " ",
			time = os.date("%I:%M %p") or " ",
			size = " ",
			desc = " ",
			entities = tostring(table.Count(dupe.Entities)) or "0",
			constraints = tostring(#dupe.Constraints) or "0"
		}, ply)
	...
	function TOOL:Deploy()
		...
		-- Add dummy data to satisfy gm_express
		express.Send("AdvDupe2_StartGhosting", {start = true}, ply) -- Dummy data because gm_express whines when we send empty data.
