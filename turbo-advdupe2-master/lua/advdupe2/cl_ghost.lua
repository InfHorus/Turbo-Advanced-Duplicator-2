...
AdvDupe2.HeadOffset = Vector()
AdvDupe2.HeadAngle = Angle()
...
function AdvDupe2.UpdateGhosts(force)
    ...
    local Lheadpos = AdvDupe2.HeadGhost:GetPos()
    local Lheadang = AdvDupe2.HeadGhost:GetAngles()
    ...
    if math.abs(Lheadpos.x - headpos.x) > 0.01 or
       ...
       math.abs(Lheadang.r - headang.r) > 0.01 or force then
        ...
        for k, ghost in pairs(AdvDupe2.GhostEntities) do
            if IsValid(ghost) and ghost.Phys then
                local pos, ang = LocalToWorld(ghost.Phys.Pos, ghost.Phys.Angle, originpos, originang)
                ghost:SetPos(pos)
                ghost:SetAngles(ang)
            end
        end
...
hook.Add("ExpressLoaded", "AdvDupe2_GhostSetup", function()
    express.Receive("AdvDupe2_SendGhosts", function(data)
        AdvDupe2.RemoveGhosts()
        AdvDupe2.GhostToSpawn = {}
        ...
        -- Reconstruct the ghost data
        for i = 1, #data.ghostData do
            local ghost = data.ghostData[i]
            AdvDupe2.GhostToSpawn[i] = {
                Model = cache[ghost.model],
                PhysicsObjects = ghost.physicsObjects
            }
        end
    ...
    express.Receive("AdvDupe2_AddGhost", function(data)
        local ghost = {
            Model = data.Model,
...
hook.Add("ExpressLoaded", "AdvDupe2_GhostControls", function()
    express.Receive("AdvDupe2_StartGhosting", function()
        AdvDupe2.StartGhosting()
    end)

    express.Receive("AdvDupe2_RemoveGhosts", function()
        AdvDupe2.RemoveGhosts()
    end)
end)

if CLIENT then
    -- Add this to ensure UpdateGhosts is available globally
    _G.AdvDupe2 = _G.AdvDupe2 or {}
    _G.AdvDupe2.UpdateGhosts = AdvDupe2.UpdateGhosts
end